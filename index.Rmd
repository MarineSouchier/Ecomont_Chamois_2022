...
Coeff_glm1 <-round(((1/exp(summary(glm1)$coefficients[2]))-1)*100, 3)
...
En calculant l'inverse de la fonction logit, on calcule le coefficient qui permet d'exprimer la fécondité annuelle en fonction de l'âge.
...
Coeff_glm2 <-round(((1/exp(summary(glm2)$coefficients[2]))-1)*100, 3)
...
Pour appuyer ce choix, sur le graphique qui représente les données groupées par âge, un modèle de type quadratique est ajoutée et ce modèle est bien ajustée aux données observées (cf figure ci-dessous).
...
Par contre, on peut conclure que la variable "age^2" a un bien un effet significatif et négatif sur la fécondité annuelle des chamois comme l'indique l'exponentielle du logarithme de l'odd-ratio qui est <1.
...
summary(glm1g)
exp(summary(glm1g)$coefficients[[2]])
```
...
```
Le modèle glm1c présente le plus faible AIC.
La variable "âge" a un effet significatif négatif sur la fécondité annuelle via ce modèle ce qui n'est pas surprenant d'après les observations graphiques. 
...
### Variation de l'age moyen des chamois en fonction des années
...
tailleeffetlm2 <- coefficients(lm2)[2]*26
```
Le modèle linéaire semble valider toutes les hypothèses requises: 
  -Normalité des résidus validée
  -Homoscédasticité des résidus non validée
La p value est < 0.05 donc l'effet observé est significatif. Lorsque la taille de l'effet est évalué, l'age moyen de la population augmente de `r tailleeffetlm2` sur les 26 années d'étude donc la taille de l'effet est faible.
...
summary(glm4)
```
On observe via ce modèle bien l'effet de l'age et l'absence d'effet des années.
Ainsi, la fécondité de la population de chamois n'est pas sensible à la variation de la taille de la population qui n'a donc surement pas atteint sa capacité de charge maximum.
...
cham_id = cham2 %>% 
  group_by(id) %>% 
  dplyr::summarise(feconditetotale= sum(fec), long=long, agemax = max(age), pds=pds, coh=coh, anneetot=n(), minan=min(year), maxan=max(year), agemark=agemark, ratiosuivi=anneetot/long) %>%   unique()
```
...
plot11 <- ggplot(cham_long, aes(x=long, y=feconditetotale)) +
    ...
    geom_smooth(); plot11
```
Le graphique ci-dessous est biaisé du fait que les individus ne sont pas suivis toute la durée de leur vie et donc la fécondité totale ne représente que la somme de la fécondité sur les années de suivi.
...
```{r message=FALSE, warning=FALSE, echo=FALSE}
plot12 <- ggplot(cham_long, aes(x=anneetot)) +
...
```{r message=FALSE, warning=FALSE, echo=FALSE}
hist(cham_long$agemark, breaks=126, xlim=c(0,16), ylim=c(0,20), xlab = "Age marquage", main="Nombre individus en fonction de l'âge de marquage")
```
...
Une sélection des individus suivis sur 60 ou 70% de leur vie est réalisée en utilisant les arguments suivants:
-taille échantillon > 30 individus
-individus au moins suivis sur 60% de leur vie
...
plot15 <-ggplot(cham_long70, aes(x=long, y=feconditetotale)) +
    geom_point() +
    labs(title = "Fécondité totale pour les individus suivis sur 70% de leur vie",x="Longévité", y="Fécondité totale") +
    theme(plot.title = element_text(hjust = 0.5)) +
    geom_smooth(); plot15

plot16 <-ggplot(cham_long60, aes(x=long, y=feconditetotale)) +
    geom_point() +
    labs(title = "Fécondité totale pour les individus suivis sur 60% de leur vie",x="Longévité", y="Fécondité totale") +
    theme(plot.title = element_text(hjust = 0.5)) +
    geom_smooth(); plot16
```
...
Le modèle appliqué aux individus suivis sur au moins 60% de leur vie indique un effet significatif de la longévité sur la fécondité totale  avec une augmentation de 0.76 individus par point de longévité supplémentaire (R2>0.65).
Le modèle appliqué aux individus suivis sur au moins 70% de leur vie indique un effet significatif de la longévité sur la fécondité totale avec une augmentation de 0.90 individus par point de longévité supplémentaire (R2>0.67).
